...
#include "special_functions.hip.h"
...
void gpu_isf(double *isf, double *qvecs, double *beads, double norm, double norm2, double inorm,
    ...
    for (int tile = 0; tile < hipGridDim_x; tile++) {
        ...
        int s_beads_idx = tile * hipBlockDim_x + hipThreadIdx_x;
        if (s_beads_idx < full_number_of_beads) {
            for (int j = 0; j < NDIM; j++) {
                s_beads[hipThreadIdx_x*NDIM + j] = beads[s_beads_idx*NDIM + j];
            }
        }
        __syncthreads();
        ...
        if (i < full_number_of_beads) {
            ...
            for (int j = 0; j < GPU_BLOCK_SIZE; j++) {
                s_beads_idx = tile * hipBlockDim_x + j;
                if (s_beads_idx < full_number_of_beads) {
...
void gpu_isf3(double *isf, double *qmags, double *beads, int qmag_idx, int tau_idx, double inorm,
    ...
    if (i < NNM) {
        ...
        if (bead1_idx != bead2_idx) {
            ...
            s_isf[hipThreadIdx_x] = sin(q_dot_sep)*inorm/q_dot_sep;
        } else {
            s_isf[hipThreadIdx_x] = inorm;
        }
    } else {
...
void gpu_isf_dse_mean(double *isf, double *qmags, double *beads, int qmag_idx, int tau_idx, double inorm,
    ...
    if (i < NNM) {
        ...
        if (bead1_idx != bead2_idx) {
            ...
            si(ar,&_si_ar);
            si(br,&_si_br);
            s_isf[hipThreadIdx_x] = inorm*(_si_ar - _si_br)/(ar - br);
        } else {
            s_isf[hipThreadIdx_x] = inorm;
        }
    } else {
...
void gpu_isf_set_separations(
    ...
    int i, j, separation_idx, tau_sep, bead1_idx, bead2_idx, true_bead1_idx, true_bead2_idx, tau_idx_first_bead,
        particle_idx_first_bead, tau_idx_second_bead, particle_idx_second_bead;
    int t_idx = hipBlockDim_x * hipBlockIdx_x + hipThreadIdx_x;
...
void gpu_isf_set_isf(
    ...
    for (int i=0; i<number_of_qvecs; i++) {
        ...
        if (t_idx < number_of_separations) {
            gpu_isf_set_isf_piece(s_isf + hipThreadIdx_x, qvecs + i*NDIM, separations + separation_idx*NDIM, number_of_separations, 2.0/number_of_beads);
            if (t_idx == 0) {
                 s_isf[hipThreadIdx_x] += 1.0;
            }
        } else {
            s_isf[hipThreadIdx_x] = 0.0;
        }
        __syncthreads();
        gpu_reduce(s_isf, hipThreadIdx_x);
        if (hipThreadIdx_x == 0) {
        ...
        for (int j=1; j<number_of_timeslices/2; j++) {
            ...
            if (t_idx < number_of_separations) {
                gpu_isf_set_isf_piece(s_isf + hipThreadIdx_x, qvecs + i*NDIM, separations + separation_idx*NDIM, number_of_separations, 1.0/number_of_beads);
            } else {
                s_isf[hipThreadIdx_x] = 0.0;
            }
            __syncthreads();
