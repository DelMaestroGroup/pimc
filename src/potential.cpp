...
/**************************************************************************//**
 * Constructor.
 * _sigma and _epsilon are the LJ potential of the atoms we are simulating
 * For 4He these are: σ=2.6413813 Å and ε=10.956 K
******************************************************************************/
FixedPositionLJPotential::FixedPositionLJPotential (const double _sigma, const double _epsilon, 
    ...
    /* We assume a specific structure for the fixed input file:  
     * # chemicalsymbol_0=0, ..., chemical_symbol_numAtomTypes=numAtomTypes-1
     * numAtomTypes numFixedAtoms
     *
     * sigma(0) epsilon(0)
     * ⋮ 
     * sigma(numAtomTypes-1) epsilon(numAtomTypes-1)
     *
     * atomType(0) fixedAtom(0)[0] fixedAtom(0)[0] fixedAtom(0)[2]
     * ⋮ 
     * atomType(0) fixedAtom(0)[0] fixedAtom(0)[0] fixedAtom(0)[2]
     */
    auto& fixedStream = communicate()->file("fixed")->stream();

    /* Ignore any comment lines */
    if (fixedStream.peek() == '#') 
        fixedStream.ignore(std::numeric_limits<std::streamsize>::max(), '\n');

    /* Read the number of atom types and the number of atoms */
    fixedStream >> numAtomTypes >> numFixedAtoms;

    /* Resize the internal arrays */
    sigma.resize(numAtomTypes);
    epsilon.resize(numAtomTypes);
    fixedAtoms.resize(numFixedAtoms); 
    atomType.resize(numFixedAtoms);    

    /* Now we get the bare σ and ϵ values */
    for (int n = 0; n < numAtomTypes; ++n) {
        if (!(fixedStream >> sigma(n) >> epsilon(n))) {
            throw std::runtime_error("Failed to read sigma/epsilon");
        }
    }

    /* Load the atom types and positions */
    for (int n = 0; n < numFixedAtoms; ++n) {
        if (!(fixedStream >> atomType(n))) {
            throw std::runtime_error("Failed to read atom type");
        }
        for (int i = 0; i < NDIM; ++i) {
            if (!(fixedStream >> fixedAtoms(n)[i])) {
                throw std::runtime_error("Failed to read atom coordinate");
            }
        }
    }

    /* Perform the Berthlot mixing for the atoms we are simulating */
    for (int n = 0; n < numAtomTypes; ++n) {
        sigma(n) = 0.5 * (sigma(n) + _sigma);  // Lorentz
...
/**************************************************************************//**
 *  Return the value of the LJ interaction between an atom and all fixed
 *  particles
...
double FixedPositionLJPotential::V(const dVec &r) {
    ...
    for (int n = 0; n < numFixedAtoms; n++) {
        ...
        if (x2 >= EPS) {
            ...
            double ix12 = ix6 * ix6;
            v += 4.0*epsilon(atomType(n))*(ix12-ix6);
        }

    }
    return std::min(v, 500.0);
