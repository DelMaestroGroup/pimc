...
template <bool ZERO_FREQUENCY = false>
__global__ void gpu_isf(double* __restrict__ isf, double* __restrict__ qvecs, double *beads, double inorm, int M, int N, int N_extent, double maxR) {
    __shared__ double s_isf[GPU_BLOCK_SIZE]; // temporarily store isf on shared memory of gpu
...
void gpu_ssf_launcher(double* __restrict__ ssf, double* __restrict__ qvecs, double *beads, double inorm, int M, int N, int N_extent, int n_qvecs) {
    gpu_ssf<<<dim3(n_qvecs), dim3(GPU_BLOCK_SIZE), 0, 0>>>(ssf, qvecs, beads, inorm, M, N, N_extent);
}
void gpu_ssf_launcher(cudaStream_t s, double* __restrict__ ssf, double* __restrict__ qvecs, double *beads, double inorm, int M, int N, int N_extent, int n_qvecs) {
