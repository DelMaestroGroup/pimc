...
def main(): 
	...
	#outName += '-t-%7.5f.dat' % float(pimc.params[pimc.id[0]]['Imaginary Time Step'])
	outName += '.dat'
	...
	if options.plot:
		...
		numParams = len(param)
		markers = loadgmt.getMarkerList(numParams)
		...
		for n in range(numParams):
			lab = '%s = %s' % (options.reduce,param[n])
			errorbar(x2[n,:], ave2[n,:], yerr=err2[n,:],color=colors[n],marker=markers[n],\
		...
		if options.gce: 
			...
			for n in range(numParams): 
				lab = '%s = %s' % (options.reduce,param[n]) 
				errorbar(x4[n,:], ave4[n,:], err4[n,:],color=colors[n],marker=markers[n],\
						markeredgecolor=colors[n], markersize=8,linestyle='None',label=lab) 
		...
		if len(glob.glob('CYLINDER')) > 0:
			...
			for n in range(numParams):
				lab = '%s = %s' % (options.reduce,param[n])
				errorbar(x3[n,:], (ave3[n,:]+1.0E-15), err3[n,:],color=colors[n],marker=markers[n],\
						markeredgecolor=colors[n], markersize=8,linestyle='None',label=lab)
